...
static void tracking_event_cb();
static void wifi_destory();
static Tracking *tracker;
...
public:
    ...
    void hide(lv_icon_status_bar_t icon)
    {
...
public:
    ...
    void createMenu(lv_menu_config_t *config, int count, lv_event_cb_t event_cb, int direction = 1)
    {
        ...
        lv_style_set_image_recolor_opa(&menuStyle, LV_STATE_DEFAULT, LV_OPA_COVER);
...
MenuBar::lv_menu_config_t _cfg[] = {
    ...
    {.name = "Tracking",  .img = (void *) &gps_satellite, .event_cb = tracking_event_cb },
...
class Compass {
    ...
    float calculateDistance(float flat1, float flon1, float flat2, float flon2) {
        ...
        dist_calc = (sinf(diflat/2.0f)*sinf(diflat/2.0f));
        dist_calc2= cosf(flat1);
        dist_calc2*=cosf(flat2);
        dist_calc2*=sinf(diflon/2.0f);
        dist_calc2*=sinf(diflon/2.0f);
        dist_calc +=dist_calc2;

        dist_calc=(2*atan2f(sqrtf(dist_calc), sqrtf(1.0f-dist_calc)));
    ...
    float calculateDirection(float otherLatitude, float otherLongitude, float myLatitude, float myLongitude) {
        ...
        myLongitude = myLongitude * PI / 180.f;
    ...
    void update() {
        ...
        else if (!tracker->getMyPosition(myPosition)) {
            lv_label_set_text(distanceLabel, "No GPS fix");
        }
        else if (!tracker->getRemotePosition(otherPosition)) {
            lv_label_set_text(distanceLabel, "No remote GPS");
        }
        else if (!myPosition.valid || !otherPosition.valid) {
            lv_label_set_text(distanceLabel, "Data invalid");
        }
        else  {
            // at this point everything should be valid
            float myHeading = tracker->getHeading();
            float distance = calculateDistance(otherPosition.lat, otherPosition.lng, myPosition.lat, myPosition.lng);
            float direction = calculateDirection(otherPosition.lat, otherPosition.lng, myPosition.lat, myPosition.lng);
            direction += myHeading;
            if (distance > 1000) 
                sprintf(distanceStr, "%.1f km", distance/1000.f);
            else
                sprintf(distanceStr, "%.0f m", distance);
...
void tracking_sw_event_cb(uint8_t index, bool en) {
    if (en) {
        tracker->enable();
        if (tracker->isEnabled())
            Serial.println("Tracking enabled");
        else
            Serial.println("Tracking NOT enabled");

        bar.show(LV_STATUS_BAR_GPS);
    }
    else {
        tracker->disable();
        bar.hide(LV_STATUS_BAR_GPS);
    }
}

static void tracking_event_cb()
{
    Switch::switch_cfg_t cfg[] = {{"Enable", tracking_sw_event_cb}};
    sw = new Switch;
    sw->create(cfg, 1, []() {
        delete sw;
        sw = nullptr;
