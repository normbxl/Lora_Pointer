...
void low_energy()
{
    if (ttgo->bl->isOn()) {
        ...
        ttgo->displaySleep();
        if (!WiFi.isConnected() && !tracker->isEnabled()) {
            lenergy = true;
            WiFi.mode(WIFI_OFF);
...
void setup()
{
    ...
    ttgo->rtc->syncToSystem();
    Serial.println("ttgo->rtc->syncToSystem()");
    ...
    */
    ...
    // setting up S7XG
    if (tracker->setup()) {
        Serial.println("Tracker set up");
    }
    else
        Serial.println("Tracker NOT set up");
